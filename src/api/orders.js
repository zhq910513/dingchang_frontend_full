...
function sanitizeImages(images) {
  ...
  for (const img of images) {
    ...
    const order_image_id =
      img.order_image_id !== undefined && img.order_image_id !== null ? toValidId(img.order_image_id) : undefined;
    const image_file_id =
      img.image_file_id !== undefined && img.image_file_id !== null ? toValidId(img.image_file_id) : undefined;

    const storage_key =
      img.storage_key != null ? String(img.storage_key).trim().replace(/^\/+/, "") : "";
    const md5 = img.md5 != null ? String(img.md5).trim().toLowerCase() : "";

    if (!order_image_id && !image_file_id && !storage_key) continue;
