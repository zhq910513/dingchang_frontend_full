...
function formatYmdLocal(d) {
  // ✅ 用本地日历日输出 YYYY-MM-DD（避免 toISOString() 的 UTC 跨日坑）
...
function normalizeYmd(v) {
  ...
  if (isValidDate(v)) return formatYmdLocal(v);
  if (typeof v === "string") {
    const s = v.trim();
    return s || "";
  }
    ...
    if (Array.isArray(v)) {
      const arr = v
        .map((x) => {
          ...
          if (isValidDate(x)) return formatYmdLocal(x);
          return x;
        })
